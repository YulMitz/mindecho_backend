...
*/
export const storeResponseAndMetadata = async (userId, chatbotType, response) => {
    try { 
        // Find active session metadata in the database
        const activeSession = await ChatSession.findOne({ userId: userId, chatbotType: chatbotType }).exec();
        ...
        if (activeSession) {
            ...
            console.log('New chat session metadata saved to database:', chatSession);
        }
